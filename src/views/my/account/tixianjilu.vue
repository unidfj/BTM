<template>
  <!-- 提现记录 -->
  <div class="page">
    <m-header title="提现记录" :canback="Boolean(1)"></m-header>
    <section class="body">
      <!-- <div class="tab">
      ...
      <div class="listbox" :style="'height:'+(screenHeight-40)+'px;overflow:auto;'" v-lazy-load="lazyLoad">
        ...
        > -->
          <div class="tabitem" v-for="(item, index) of list" :key="index" >
            <div class="list">
              <span>币种：{{item.moneytype}}</span>
              <span>数量：{{item.money}}</span>
            </div>
            <div class="list">
              <span>方式：{{item.state}}</span>
            ...
            <div class="list">
              <span class="reason" v-if='item.reason'>理由：{{item.reason}}</span>
            </div>
